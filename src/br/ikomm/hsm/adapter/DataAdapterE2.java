...
public class DataAdapterE2 extends BaseAdapter {

	private Context context;
	private LayoutInflater inflater;
	private List<Palestra> palestras;

	public DataAdapterE2(Activity activity, Context context, int id) {
		super();
		this.context = context;
		inflater = LayoutInflater.from(activity);
		List<Palestra> all = new PalestraRepository().getAllNovo2();

		palestras = new ArrayList<Palestra>();

		for (int i = all.size() - 1; i >= 0; i--) {
			if (id == 1){
				if (all.get(i).day.equals("1 de abril")) {
					palestras.add(all.get(i));
				}
			} else if (id == 2){
				if (all.get(i).day.equals("2 de abril")) {
					palestras.add(all.get(i));
				}
			}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (palestraAtual.id == null || palestraAtual.id.isEmpty()) {
			...
			horaInicio.setText(palestraAtual.hour_final);

			TextView horaFinal = (TextView) view
					.findViewById(R.id.tHorarioFimD1);
			horaFinal.setText(palestraAtual.hour_init);
