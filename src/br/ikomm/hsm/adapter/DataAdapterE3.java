...
public class DataAdapterE3 extends BaseAdapter {

	private Context context;
	private LayoutInflater inflater;
	private List<Palestra> palestras;

	public DataAdapterE3(Activity activity, Context context, int id) {
		super();
		...
		for (int i = all.size() - 1; i >= 0; i--) {
			if (id == 1){
				if (all.get(i).day.equals("6 de maio")) {
					palestras.add(all.get(i));
				}
			} else if (id == 2){
				if (all.get(i).day.equals("7 de maio")) {
					palestras.add(all.get(i));
				}
			}
